...
class PointerApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        app_menu.add_command(
            label='Novo ticket', command=self.pop_up_new_ticket
        )
        ...
        budget_entry = ttk.Entry(
            entry_frame, textvariable=self.budget, width=15, justify='right'
        )
        ...
        if self.icon_search:
            ttk.Button(
                entry_frame, image=self.icon_search, command=self.load_table
            ).pack(side='left', padx=5)
        else:
            ttk.Button(
                entry_frame, text='Buscar', command=self.load_table
            ).pack(side='left', padx=5)

        if self.icon_new_ticket:
            ttk.Button(
                entry_frame,
                image=self.icon_new_ticket,
                command=self.pop_up_new_ticket,
            ).pack(side='left', padx=5)
        else:
            ttk.Button(
                entry_frame, text='Novo ticket', command=self.pop_up_new_ticket
            ).pack(side='left', padx=5)
        ...
        if self.icon_add_ticket:
            ttk.Button(
                entry_frame,
                image=self.icon_add_ticket,
                command=self.pop_up_new_ticket,
            ).pack(side='left', padx=5)
        else:
            ttk.Button(
                entry_frame, text='+', command=self.pop_up_new_ticket
            ).pack(side='left', padx=5)
        ...
        self.table = ttk.Treeview(
            treeview_frame, columns=columns, show='headings', height=18
        )
        for col in columns:
            self.table.heading(
                col, text=col, command=partial(self.sort_column, col)
            )
            anchor = 'center'
        ...
        vsb = ttk.Scrollbar(
            treeview_frame, orient='vertical', command=self.table.yview
        )
    ...
    def pop_up_new_ticket(self):
        ...
        def save():
            ...
            if self.register_ticket(ativo):
                pop_up_win.destroy()
                messagebox.showinfo(
                    'Sucesso', f'{ativo} cadastrado com sucesso.'
                )

        btn_frame = ttk.Frame(pop_up_win)
        btn_frame.pack(pady=12)
        ttk.Button(btn_frame, text='Salvar', command=save).pack(
            side='left', padx=8
        )
        ttk.Button(
            btn_frame, text='Cancelar', command=pop_up_win.destroy
        ).pack(side='left')
    ...
    def search_worker(self, ticker: str) -> Tuple[str, dict | str | None]:
        ...
        try:
            ...
            # decide tag
            if divs_ult12 > price * 0.15:  # yield > 15% do preço
                tag = 'verde'
            elif divs_ult12 > price * 0.10:  # yield > 10% do preço
                tag = 'amarelo'
            else:
                tag = 'vermelho'
